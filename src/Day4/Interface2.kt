...
fun main(){
    dragObjects(arrayOf(CircleNew(4.3), SquareNew(4.3), TriangleNew(4.3, 2.4), Playerz("Hari")))
...
fun dragObjects(objects : Array<Draggables>){     //Polymorphism is implemented -> Parents can hold the reference of its and can call the methods of its child classes which are common
    for(obj in objects){
        obj.drag()
    }
}
...
abstract class ShapeNew : Draggables {
...
class Player(val name :String) : Draggables {
